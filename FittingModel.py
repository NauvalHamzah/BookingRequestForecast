...
for item in Vehicle_Type:
    print(item)
    source1 = source[['Date',item]]

    train = source1[source1['Date'] <= pd.to_datetime("2024-01-31", format='%Y-%m-%d')]
    test = source1[source1['Date'] > pd.to_datetime("2024-01-31", format='%Y-%m-%d')]

    import pmdarima as pm

    # Assuming your time series data is in 'data'
    model = pm.auto_arima(train[item], seasonal=True, m=7,  # assuming monthly seasonality
...
np.savetxt('BestParameters.csv', best_parameters, delimiter=',')
