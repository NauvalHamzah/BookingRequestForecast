...
train = source[source['Date Hour'] <= pd.to_datetime("2024-01-31 00:00:00", format='%Y-%m-%d %H:%M:%S')]
test = source[source['Date Hour'] > pd.to_datetime("2024-01-31 00:00:00", format='%Y-%m-%d %H:%M:%S')]

Pred_result = test.copy()
Pred_result.loc[:,Pred_result.columns != 'Date Hour']=0
...
Pred_result.to_csv('HourlyForecast.csv', index=False)
